...
import { resolveTerminalFontFamily } from "@posthog/ui/features/terminal/resolveTerminalFontFamily";
...
export function Terminal({
  ...
  useEffect(() => {
    if (!sessionId) return;
    const dataSub = getShellClient().onData(sessionId, (event) => {
      terminalManager.writeData(event.sessionId, event.data);
    });
    const exitSub = getShellClient().onExit(sessionId, (event) => {

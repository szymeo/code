...
import { DEFAULT_TERMINAL_FONT_FAMILY } from "./resolveTerminalFontFamily";
...
class TerminalManagerImpl {
  ...
  private async initializeSession(
    ...
    try {
      const sessionExists = await getShellClient().check({ sessionId });
      if (!sessionExists) {
        ...
        if (command && cwd) {
          await getShellClient().createCommand({
            sessionId,
            command,
            cwd,
            taskId,
          ...
          await getShellClient().create({ sessionId, cwd, taskId });
        }
      }

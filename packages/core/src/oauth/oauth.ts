...
import { OAUTH_CALLBACK, OAUTH_ENV, OAUTH_LOGGER } from "./identifiers";
import type { OAuthCallbackReceiver, OAuthEnv, OAuthLogger } from "./ports";
import type {
  CancelFlowOutput,
  CloudRegion,
  OAuthTokenResponse,
...
export class OAuthService {
  ...
  constructor(
    ...
    @inject(OAUTH_CALLBACK)
    private readonly callbackServer: OAuthCallbackReceiver,
    @inject(OAUTH_ENV)
    private readonly env: OAuthEnv,
    @inject(OAUTH_LOGGER)
    private readonly log: OAuthLogger,
  ...
  private async waitForHttpCallback(
    ...
    try {
      return await this.callbackServer.waitForCode({

...
interface MockActivityMonitor extends UsageActivityMonitor {
...
function makeThresholdStore(): ThresholdStore {
  return {
...
function makeService(
  ...
  activity: UsageActivityMonitor,
): UsageMonitorService {
  return new UsageMonitorService(
    gateway,
    activity,
    makeThresholdStore(),
    makeLogger(),
...
function mockGateway(usage: UsageOutput | null): UsageGateway {
  return {
    fetchUsage: vi.fn().mockResolvedValue(usage),
  } as unknown as UsageGateway;
}

...
export class UsageMonitorService extends TypedEventEmitter<UsageMonitorEvents> {
  ...
  constructor(
    @inject(USAGE_GATEWAY)
    private readonly llmGateway: UsageGateway,
    @inject(USAGE_ACTIVITY_MONITOR)
    private readonly activity: UsageActivityMonitor,
    @inject(USAGE_THRESHOLD_STORE)
    private readonly thresholdStore: ThresholdStore,
    @inject(USAGE_LOGGER)
    private readonly log: UsageLogger,
  ) {
    super();
    this.thresholdsSeen = { ...this.thresholdStore.getThresholdsSeen() };
  }
  ...
  init(): void {
    ...
    this.activity.onLlmActivity(this.onLlmActivity);
    void this.fetchOnce();
  ...
  @preDestroy()
  stop(): void {
    this.activity.offLlmActivity(this.onLlmActivity);
  ...
  private maybeEmit(
    ...
    this.thresholdStore.setThresholdsSeen(this.thresholdsSeen);

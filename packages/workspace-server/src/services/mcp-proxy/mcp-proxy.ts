...
import { MCP_PROXY_AUTH, MCP_PROXY_LOGGER } from "./identifiers";
import type { McpProxyAuth, McpProxyLogger } from "./ports";
...
export class McpProxyService {
  ...
  constructor(
    ...
    @inject(MCP_PROXY_LOGGER)
    private readonly log: McpProxyLogger,
  ) {}
  ...
  private async forwardRequest(
    ...
    try {
      ...
      this.log.error("MCP proxy forward error", { id, url, err });
      if (!res.headersSent) {
        res.writeHead(502);
      }
      res.end("Proxy error");
